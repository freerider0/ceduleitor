    ...
    var body: some View {
        List {
            if logger.sessions.isEmpty {
                ...
                ForEach(logger.sessions) { session in
                    NavigationLink(destination: VideoReplayView(session: session, logger: logger)) {
                        sessionRow(session)
                    }
                }
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                HStack(spacing: 16) {
                    // Room capture button
                    NavigationLink(destination: RoomCaptureViewImproved()) {
                    ...
                    NavigationLink(destination: ARRecordingView()) {
