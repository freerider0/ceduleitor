...
public class System {
    ...
    public func addConstraint(_ constraint: Constraint) {
        ...
        if parameters.debugMode {
            print("DEBUG: Added constraint of type \(constraint.type) - Total constraints: \(allConstraints.count)")
            print("DEBUG: Constraint has \(constraint.parameters.count) parameters")
        }
    }
    ...
    public func solve() -> SolveStatus {
        if parameters.debugMode {
            print("DEBUG: System.solve() called")
            print("DEBUG: Total constraints: \(allConstraints.count)")
            print("DEBUG: Total subsystems: \(subSystems.count)")
            for (i, subSystem) in subSystems.enumerated() {
                print("DEBUG: SubSystem \(i) has \(subSystem.constraints.count) constraints and \(subSystem.parameterRefs.count) parameters")
            }
        }
        
        if allConstraints.isEmpty {
            if parameters.debugMode {
                print("DEBUG: No constraints to solve, returning success")
            }
        ...
        for (index, subSystem) in subSystems.enumerated() where !subSystem.isEmpty() {
            if parameters.debugMode {
                print("DEBUG: Solving subsystem \(index) with \(subSystem.constraints.count) constraints")
            }
            let (status, iterations) = solveSubSystem(subSystem)
            totalIterations += iterations
            
            if parameters.debugMode {
                print("DEBUG: Subsystem \(index) result: \(status), iterations: \(iterations)")
            }
    ...
    private func solveSubSystem(_ subSystem: SubSystem) -> (status: SolveStatus, iterations: Int) {
        ...
        if parameters.debugMode {
            print("DEBUG: solveSubSystem called with \(initialParams.count) parameters")
            if initialParams.isEmpty {
                print("DEBUG: WARNING - No parameters to solve!")
                return (.success, 0)
            }
        }
    ...
    public func getConstraintCount() -> Int {
        return allConstraints.count
    }
    
    public func getParameterCount() -> Int {
        return subSystems.reduce(0) { $0 + $1.parameterRefs.count }
    }
}